...
- **NDMF/MAのエラーが出る**: `GonUnsheathSystem` のInspectorで **[Troubleshoot / Auto Fix]** を実行してください。  
  （Consoleのエラーメッセージを読み取り、MA Menu Installerのインストール先未設定やExpression Parametersの型競合をBoolへ統一する処理などを自動で修正します）
